	...
	with open(f'../trace/trace.csv', "r") as t:
		...
		with open(sim_file, "w") as f:
			...
			for incoming_request in requests:
				...
				while stats["time"] < float(incoming_request["time"]):
								...
								for shard_id in shard_ids:
									if instances[workflow_id]["shards"][shard_id]["process"] != None:
										...
										if float(instances[workflow_id]["shards"][shard_id]["process"]["time"]) + float(instances[workflow_id]["shards"][shard_id]["process"]["latency"]) < stats["time"]: # request done, returning to idle
											...
											if instances[workflow_id]["shards"][shard_id]["owner"] == "pileup": # final function executed, shutting down (we know what the last function is beforehand)
												to_delete = True
											else:
								...
								for instance_id, targets in instance_ids.items():
									for target, c in targets.items():
										if instance_id in instances[function_name]:
											if float(instances[function_name][instance_id]["process"][target]["time"]) + float(instances[function_name][instance_id]["process"][target]["latency"]) < stats["time"]: # request done, returning to idle
				...
				if args.scheduler == "arbiter":
					...
					if len(no_process[wf_id]) > 0: # find pre-existing idle shard for incoming request
						shard_id = no_process[wf_id].pop(next(iter(no_process[wf_id])))
					...
					if len(no_process[function_name]) > 0: # find pre-existing idle instance for incoming request
						...
						prev_request = None
						if target in instances[function_name][instance_id]["process"]:
							prev_request = instances[function_name][instance_id]["process"][target]
						...
						if args.assignment == "standard" and prev_request is None:
							allocated[instances[function_name][instance_id]["owner"][target]] += container_alloc[function_name] / args.target
						elif args.assignment == "exact":
							if prev_request != None:
								instances[function_name][instance_id]["allocated"] -= float(prev_request["mem"])
								allocated[instances[function_name][instance_id]["owner"][target]] -= float(prev_request["mem"])
						...
						next_finish = str(int(math.ceil(float(instances[function_name][instance_id]["process"][target]["time"]) + float(instances[function_name][instance_id]["process"][target]["latency"]))))
						next_ttl = str(int(math.ceil(instances[function_name][instance_id]["last_invoked"] + args.ttl)))
						if next_ttl < next_finish:
							next_ttl = next_finish
						if next_finish not in next_event:
							next_event[next_finish] = {}
						if function_name not in next_event[next_finish]:
							next_event[next_finish][function_name] = {}
						if instance_id not in next_event[next_finish][function_name]:
							next_event[next_finish][function_name][instance_id] = {}
						if next_ttl not in next_event:
							next_event[next_ttl] = {}
						if function_name not in next_event[next_ttl]:
							next_event[next_ttl][function_name] = {}
						if instance_id not in next_event[next_ttl][function_name]:
							next_event[next_ttl][function_name][instance_id] = {}
						next_event[next_finish][function_name][instance_id][target] = 1
						next_event[next_ttl][function_name][instance_id][target] = 1
						...
						stats["used_total"] += float(incoming_request["mem"])
						stats["used_" + function_name] += float(incoming_request["mem"])
						stats["idle_total"] += container_alloc[function_name] - float(incoming_request["mem"])
						stats["idle_" + function_name] += container_alloc[function_name] - float(incoming_request["mem"])
						stats["alloc_total"] += container_alloc[function_name]
						stats["alloc_" + function_name] += container_alloc[function_name]
						next_finish = str(int(math.ceil(float(instances[function_name][instance_id]["process"]["0"]["time"]) + float(instances[function_name][instance_id]["process"]["0"]["latency"]))))
						next_ttl = str(int(math.ceil(instances[function_name][instance_id]["last_invoked"] + args.ttl)))
						if next_ttl < next_finish:
							next_ttl = next_finish
						if next_finish not in next_event:
						...
						if instance_id not in next_event[next_ttl][function_name]:
							next_event[next_ttl][function_name][instance_id] = {}
						next_event[next_finish][function_name][instance_id]["0"] = 1
						next_event[next_ttl][function_name][instance_id]["0"] = 1
							...
							for shard_id in shard_ids:
								if instances[workflow_id]["shards"][shard_id]["process"] != None:
									...
									if float(instances[workflow_id]["shards"][shard_id]["process"]["time"]) + float(instances[workflow_id]["shards"][shard_id]["process"]["latency"]) < stats["time"]: # request done, returning to idle
										...
										stats["used_total"] -= mem_shard
										stats["used_" + instances[workflow_id]["shards"][shard_id]["owner"]] -= mem_shard
										stats["idle_total"] += mem_shard - mem_shard
										stats["idle_" + instances[workflow_id]["shards"][shard_id]["owner"]] += mem_shard - mem_shard
										stats["alloc_total"] -= mem_shard
										stats["alloc_" + instances[workflow_id]["shards"][shard_id]["owner"]] -= mem_shard
										if instances[workflow_id]["shards"][shard_id]["owner"] == "pileup": # final function executed, shutting down (we know what the last function is beforehand)
											to_delete = True
											...
											instances[workflow_id]["shards"][shard_id]["process"] = None
											no_process[workflow_id].append(shard_id)
									if to_delete:
										k_events["container"]["delete"][workflow_id] += 1
										del instances[workflow_id]
					else:
